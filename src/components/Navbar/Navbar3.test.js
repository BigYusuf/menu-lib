...
import { Navbar3 } from './index'

/*

it('renders correctly', () => {
  const tree = renderer.create(<Navbar3 />).toJSON()
  expect(tree).toMatchSnapshot()
})
*/

describe('test all navbar3', () => {

  test('test navbar3', () => {
    render(<Navbar3 text1={'home'}/>)
...
test('confirm url feature of navbar', () => {
  const {container} =render(<Navbar3 url1={'#home'} url2={'#profile'}url3={'/pen'}/>)
...
test('confirm icon feature of navbar', () => {
  const {container} =render(<Navbar3 url1={'#home'} icon1={'bx bxs-home'}url3={'/pen'}/>)
  ...
  test('to know if menunumber is correct', () => {
    const {container} = render(<Navbar3 MenuNumber={3} />);
  ...
  test('checking the onclick and active', () => {
    render(<Navbar3 text1={'home'} url1={'#home'}/>)
...
jest.mock('../NavList/NavList3', () => ({Font, MenuNumber,IconColor,textColor, Gap,activeIconColor,degree, gradcolor1, gradcolor2, activeColor, url1, icon1}) => 
{MenuNumber &&
  ...
  test("If Navbar props is called, NavList props is called", () => {
    const {container} = render(<Navbar3 Font={20} activeIconColor='blue' MenuNumber={5} 
  ...
  test("If Navbar props is not called, NavList is not called", () => {
    const {container} = render(<Navbar3 />);
    const mockComponent1 = container.querySelector('li#mock1')
 ...
 test('confirm url address of navbar component', () => {
   render(<Navbar3 url1={'https://www.test.com/'} url2={'#profile'}url3={'/pen'}/>)
