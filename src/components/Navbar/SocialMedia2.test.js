 ...
 */
 ...
 describe('test all SocialMedia2', () => {
 
   test('test SocialMedia2', () => {
     render(<SocialMedia2 text1={'facebook'}/>)
 ...
 test('confirm url feature of navbar', () => {
   const {container} =render(<SocialMedia2 url1={'#facebook'} url2={'#profile'}url3={'/pen'}/>)
 ...
 test('confirm icon feature of navbar', () => {
   const {container} =render(<SocialMedia2 url1={'#facebook'} icon1={'bx bxl-facebook'}url3={'/pen'}/>)
   ...
   test('to know if menunumber is correct', () => {
     const {container} = render(<SocialMedia2 MenuNumber={3} />);
   ...
   test('checking the mouseenter, mouseleave and active', () => {
     render(<SocialMedia2 text1={'facebook'} url1={'#facebook'}/>)
     const yulchild = screen.queryByText(/facebook/i);
     expect(yulchild).not.toBeVisible
     
    fireEvent.mouseEnter(screen.getByTestId(1))
    
    expect(yulchild).toBeVisible

    fireEvent.mouseLeave(screen.getByTestId(1))
...
{MenuNumber &&
 ...
 test("If Navbar props is called, NavList props is called", () => {
   const {container} = render(<SocialMedia2 Font={20} activeIconColor='blue' MenuNumber={5} text1='facebook'
 ...
 test("If Navbar props is not called, NavList is not called", () => {
   const {container} = render(<SocialMedia2 />);
   const mockComponent1 = container.querySelector('li#mock1')
...
test('confirm url address of navbar component', () => {
  render(<SocialMedia2 url1={'https://www.test.com/'} url2={'#profile'}url3={'/pen'}/>)
