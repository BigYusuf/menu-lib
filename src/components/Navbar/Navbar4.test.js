 ...
 */
 ...
 import { Navbar4 } from './index'
 
 /*
 
 it('renders correctly', () => {
   const tree = renderer.create(<Navbar4 />).toJSON()
   expect(tree).toMatchSnapshot()
 })
 */
 
 describe('test all navbar4', () => {
   
 test('confirm url feature of navbar', () => {
   const {container} =render(<Navbar4 url1={'#home'} url2={'#profile'}url3={'/pen'}/>)
 ...
 test('confirm icon feature of navbar', () => {
   const {container} =render(<Navbar4 url1={'#home'} iconA1={'bx bx-home'}url3={'/pen'}/>)
   
   const icon = container.getElementsByClassName('bx bx-home')
   
   expect(icon).toBeTruthy
   })
   
   test('to know if menunumber is correct', () => {
     const {container} = render(<Navbar4 MenuNumber={3} />);
     
     const boxes = container.getElementsByClassName('menu__item');
    // console.log(container); // 👉️ 2
     expect(boxes.length).toBe(3);
    });
    
    test('checking the onclick and active', () => {
      const {container} =render(<Navbar4 url1={'#home'} iconA1={'bx bx-home'} iconB1={'bx bxs-home'}url3={'/pen'}/>)
      const firstIcon = container.getElementsByClassName('bx bxs-home')
      //const secondIcon = container.getElementsByClassName('bx bx-home')
    
     expect(firstIcon).not.toBeVisible
...
jest.mock('../NavList/NavList4', () => ({Font, itemWidth,MenuNumber,IconColor,textColor, Gap,activeIconColor,degree, gradcolor1, gradcolor2, activeColor, url1, icon1}) => 
{MenuNumber &&
  ...
  test("If Navbar props is called, NavList props is called", () => {
    const {container} = render(<Navbar4 Font={20} activeIconColor='blue' MenuNumber={5} 
  ...
  test("If Navbar props is not called, NavList is not called", () => {
    const {container} = render(<Navbar4 />);
    const mockComponent1 = container.querySelector('li#mock1')
 ...
 test('confirm url address of navbar component', () => {
   render(<Navbar4 url1={'https://www.test.com/'} url2={'#profile'}url3={'/pen'}/>)
